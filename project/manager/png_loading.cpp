...
)
{
    ...
    *height = t_height;
    ...
    if (!row_pointers)
    {
        ...
        free(image_data);
        fclose(f);
        dbgprint0("png_load error: 8\n");
        return 0;
    }

    for (TUInt32 i = 0; i < t_height; ++i)
    {
    ...
    {
    case PNG_COLOR_TYPE_RGBA    : alpha = GL_RGBA;  break;
    case PNG_COLOR_TYPE_RGB     : alpha = GL_RGB;   break;
    default :
        printf( "Color type %d not supported!\n",
               png_get_color_type(png_ptr, info_ptr));
        png_destroy_read_struct(&png_ptr, &info_ptr, &end_info);
        dbgprint0("png_load error: 9\n");
        return 0;
    }
